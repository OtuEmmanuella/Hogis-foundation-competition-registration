  ...
  const updateRegistrationStatus = async (id: string, status: 'ACCEPTED' | 'REJECTED', notes?: string) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'registrations', id), {
        ...
        reviewedBy: 'Admin', // In a real app, this would be the logged-in admin's info
      });
      ...
      setRegistrations(prev => prev.map(reg => 
        reg.id === id 
          ? { ...reg, status, adminNotes: notes || '', reviewedAt: new Date().toISOString() }
          : reg
      ));
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        >
          {[
            ...
            { label: 'Pending', value: registrations.filter(r => r.status === 'PENDING').length, color: 'yellow' },
            { label: 'Accepted', value: registrations.filter(r => r.status === 'ACCEPTED').length, color: 'green' },
            { label: 'Rejected', value: registrations.filter(r => r.status === 'REJECTED').length, color: 'red' },
          ].map((stat, index) => (
